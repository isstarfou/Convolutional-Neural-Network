    ...
    def __init__(self, num_classes=10, beta1=0.9, beta2=0.999, eps=1e-8):
        ...
        for name, param in self.named_parameters():
            if param.requires_grad:
                ...
                self.v_hat[name] = torch.zeros_like(param)
        ...
        with torch.no_grad():
            for name, param in self.named_parameters():
                if param.requires_grad:
                    ...
                    # 更新最大二阶矩估计
                    self.v_hat[name] = torch.max(self.v_hat[name], self.v[name])
                    
                    # 计算偏差修正
                    m_hat = self.m[name] / (1 - self.beta1 ** self.t)
                    
                    # 更新参数
                    param.data -= lr * m_hat / (torch.sqrt(self.v_hat[name]) + self.eps)
...
def test():
    ...
    print("Weights updated with AMSGrad")
