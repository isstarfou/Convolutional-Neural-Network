...
class AdaBoundLeNet(nn.Module):
    def __init__(self, num_classes=10, beta1=0.9, beta2=0.999, eps=1e-8, final_lr=0.1, gamma=1e-3):
        super(AdaBoundLeNet, self).__init__()
        ...
        with torch.no_grad():
            for name, param in self.named_parameters():
                if param.requires_grad:
                    ...
                    # 计算自适应步长
                    step_size = lr / (torch.sqrt(v_hat) + self.eps)
                    step_size = torch.clamp(step_size, lower_bound, upper_bound)
                    
                    # 更新参数
                    param.data -= step_size * m_hat
...
def test():
    ...
    print("Weights updated with AdaBound")
