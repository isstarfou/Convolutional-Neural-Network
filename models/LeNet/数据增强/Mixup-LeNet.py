...
class MixupLeNet(nn.Module):
    def __init__(self, num_classes=10, alpha=0.2):
        super(MixupLeNet, self).__init__()
    ...
    def update_metrics(self, train_loss, train_acc, val_loss, val_acc, mixup_ratio):
        """更新评估指标"""
        self.metrics['train_loss'].append(train_loss)
        self.metrics['train_acc'].append(train_acc)
        self.metrics['val_loss'].append(val_loss)
        self.metrics['val_acc'].append(val_acc)
        self.metrics['mixup_ratio'].append(mixup_ratio)
    ...
    def print_metrics(self):
        ...
        print(f"平均混合比例: {np.mean(self.metrics['mixup_ratio']):.4f}")
    ...
    def visualize_training(self):
        ...
        # 绘制混合比例分布
        plt.subplot(2, 2, 3)
        plt.hist(self.metrics['mixup_ratio'], bins=20)
        plt.title('Mixup Ratio Distribution')
        plt.xlabel('Ratio')
    ...
    def visualize_mixup(self, x1, x2):
        """可视化Mixup效果"""
        # 转换为张量
...
def test():
    ...
    # 执行Mixup
    mixed_x, y_a, y_b, lam = model.mixup_data(x, y)
    ...
    loss = model.mixup_criterion(criterion, output, y_a, y_b, lam)
