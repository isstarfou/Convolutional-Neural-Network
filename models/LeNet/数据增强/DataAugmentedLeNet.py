...
class DataAugmentedLeNet(nn.Module):
    def __init__(self, num_classes=10):
        super(DataAugmentedLeNet, self).__init__()
    ...
    def augment_image(self, x, is_training=True, use_advanced=False):
        """增强图像"""
        if not is_training:
            return self.test_transform(x)
        
        if use_advanced:
            return self.advanced_transform(x)
        else:
    ...
    def visualize_augmentation(self, x):
        ...
        # 显示基础增强
        plt.subplot(1, 3, 2)
        aug_x1 = self.augment_image(x, use_advanced=False)
        ...
        # 显示高级增强
        plt.subplot(1, 3, 3)
        aug_x2 = self.augment_image(x, use_advanced=True)
        plt.imshow(aug_x2.permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5)
        plt.title('Advanced Augmentation')
        plt.axis('off')
...
def test():
    """测试函数"""
    # 创建模型
    model = DataAugmentedLeNet()
    
    # 创建随机输入
    x = torch.randn(3, 32, 32)
    
    # 基础增强
    base_aug_x = model.augment_image(x, use_advanced=False)
    print(f"输入形状: {x.shape}")
    print(f"基础增强后形状: {base_aug_x.shape}")
    
    # 高级增强
    adv_aug_x = model.augment_image(x, use_advanced=True)
    print(f"高级增强后形状: {adv_aug_x.shape}")
    
    # 前向传播
    output = model(base_aug_x.unsqueeze(0))
